	...
	<div class="col" style="min-width:320px;width:25%">
		<div class="card card-inverse card-primary">
			<div class="card-block">
				...
				<form name="confirm-email" method="POST" action="{{ route( 'confirm.email' ) }}">
					{{ csrf_field() }}
					...
					<div class="form-group text-center">
						<label for="email">{{ trans( 'LEC.view.confirm.vcode' ) }}</label>
					...
					<div class="form-group text-center">
						<a class="btn btn-link text-white" href="{{ route( 'confirm.repeat' ) }}">{{ trans( 'LEC.view.confirm.resend' ) }}</a>
					</div>
				</form>
			</div>
		</div>
